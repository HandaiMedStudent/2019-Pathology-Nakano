...
## （１）可逆的な変化
- 水分を吸収して肥大する；Na+,K+ ATPaseの機能不全。細胞膜の透過性が上昇→液胞の形成。  
- 脂肪化；肝細胞へのトリグリセリドの蓄積。  
- 細胞質は好酸性に傾く（赤く染色）。  
...
- 低酸素が続いたとき；Na, K ATPaseの活性低下→Na蓄積、Kの流出、浸透圧の増加、ERの拡張。解糖の増加→乳酸の蓄積、H+の蓄積。粗面小胞体からのリボソームの脱離→タンパク質合成の減少。ROS蓄積。ミトコンドリア、リソソーム膜の破壊→ネクローシス。
...
- ROSの生成；ミトコンドリアにおける酸素還元の失敗により、スーパーオキシド（O-）が生成→SODによってH2O2（より安定）に変換→Fe2+存在下で、高反応性のヒドロキシラジカルが生成。
- 白血球のファゴソーム内でもROSは生成（微生物の消化のため）＝「呼吸バースト」；H2O2から、ミエロペルオキシダーゼによって次亜塩素酸に変換。３章参照（しゃれじゃないよ）。
- ROSの除去機構；SODによるスーパーオキシドの崩壊、グルタチオンペルオキシダーゼによるH2O2の還元、カタラーゼによるH2O2の分解。ビタミンなどの抗酸化物質。
...
- 遅延性の毒物；肝臓などでのP-450による代謝で、フリーラジカルを生んでしまうもの。CCl4（四塩化炭素）、アセトアミノフェン。
